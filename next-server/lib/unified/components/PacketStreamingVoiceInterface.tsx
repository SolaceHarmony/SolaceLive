...
export const PacketStreamingVoiceInterface: React.FC<PacketStreamingVoiceInterfaceProps> = ({ 
  ...
  const [sendPartials, setSendPartials] = useState<boolean>(true);

  const packetService = useRef<PacketStreamingService | null>(null);
  // Packet path only; legacy CSM service removed
  const fallbackService = useRef<{ stopStreaming: () => void; checkCSMSupport: () => Promise<boolean>; generateStreamingResponse: (...args: any[]) => Promise<void> } | null>(null);
  ...
  const initializeServices = async () => {
    try {
      // Try to initialize packet streaming (use explicit serverUrl in Next.js env)
      const wsProto = (typeof window !== 'undefined' && window.location.protocol === 'https:') ? 'wss' : 'ws';
      const host = (typeof window !== 'undefined' && window.location.hostname) || 'localhost';
      const defaultWs = `${wsProto}://${host}:8788`;
      const envWs = (typeof process !== 'undefined' && (process as any).env?.NEXT_PUBLIC_PACKET_WS) as string | undefined;

      packetService.current = await createStreamingService({
        serverUrl: envWs || defaultWs,
        model: 'gemma3-csm-3',
        temperature: 0.8,
        maxTokens: 2048
      });
      ...
      // Legacy CSM fallback not available; surface error state
      setError('Packet streaming unavailable and legacy CSM is disabled.');
    }
  };
  ...
  const startStreamingRecognition = () => {
    if (useWhisperWasm && whisperStatus === 'ready') {
      ...
      speechService.current.startContinuousTranscription((text, isFinal) => {
        if (sendPartials && usePacketStreaming && packetService.current) {
  ...
  const handleStreamingTranscription = async (text: string, audioData: ArrayBuffer) => {
    ...
    try {
      ...
      if (streamingService) {
        ...
        // Legacy CSM disabled; surface message and return
        setError('Legacy CSM streaming is disabled. Enable packet streaming.');
      }
    } catch (err) {
      ...
      <div className="voice-interface">
        ...
        <div style={{ marginTop: 8 }}>
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            <input type="checkbox" checked={sendPartials} onChange={(e) => setSendPartials(e.target.checked)} />
            Send browser ASR partials (TEXT_PARTIAL)
          </label>
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginLeft: 16 }}>
            <input type="checkbox" checked={useWhisperWasm} onChange={(e) => setUseWhisperWasm(e.target.checked)} />
            Use Whisper WASM for partials
          </label>
        </div>
