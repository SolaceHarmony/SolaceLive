...
function getHFTokenFromRuntime(): string | null {
  // Prefer localStorage; else check window-injected; else Next/Vite public env
  ...
  try {
    // Next.js public client env (inlined at build time)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const nextToken = (typeof process !== 'undefined' ? (process as any).env?.NEXT_PUBLIC_HF_TOKEN : undefined) as string | undefined;
