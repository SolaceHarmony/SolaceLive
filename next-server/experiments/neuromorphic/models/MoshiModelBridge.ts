...
import { NeuralPacket, PacketType } from '../research/neural-packet-types';
...
export class MoshiModelBridge {
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      // Using mock transformer inside experiments to avoid production deps
  ...
  private convertInferenceToPackets(result: Omit<MoshiInferenceResult, 'processingTime'>): NeuralPacket[] {
    ...
    result.textTokens.forEach((token, index) => {
      packets.push({
        id: 'pkt-' + Date.now() + '-' + index + '-' + Math.random().toString(36).slice(2),
